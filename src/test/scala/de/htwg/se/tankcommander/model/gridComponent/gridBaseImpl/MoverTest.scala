...
class MoverTest extends FlatSpec with Matchers {
  "calcHitChance method" should "correctly calc hitchance" in {
    ...
    mover.moveTank("down")
    assert(controller.gameField.matchfieldArray(0)(1).containsThisTank.get === tank1)
    mover.moveTank("up")
    assert(controller.gameField.matchfieldArray(0)(0).containsThisTank.get === tank1)
    mover.moveTank("right")
    assert(controller.gameField.matchfieldArray(1)(0).containsThisTank.get === tank1)
    mover.moveTank("left")
    assert(controller.gameField.matchfieldArray(0)(0).containsThisTank.get === tank1)
    ...
    assert(controller.gameField.matchfieldArray(0)(2).containsThisTank.get === tank1)
